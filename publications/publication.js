...
xmlhttp.open("GET","publications/minghuo_journal.xml",false);
xmlhttp.send();
...
for (i=0;i<x.length;i++)
{
    ...
    if(x[i].getElementsByTagName("article").length!=0){
        ...
        var bibstring='@Article{mhuo_journal_'+(x.length-i).toString()+','; // initiate the bibtex item    
    ...
    if(x[i].getElementsByTagName("inproceedings").length!=0){
        ...
        document.write("</td><td>");    
        var bibstring='@Inproceedings{mhuo_proceeding_'+(x.length-i).toString()+','; // initiate the bibtex item    
...
function printauthor(au){ // this function print author's name
    ...
    // enhance "M. Huo"
    if (surname=="Huo" && firstname.indexOf("C.")!=-1){document.write("<b>")};
